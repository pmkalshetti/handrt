...
class ManoInitializer:
    ...
    def create_projection_matrices_from_K(self):
        ...
        for data in self.dataset:
            ...
            R_gl_to_cv = Rotation.from_euler("X", np.pi).as_matrix()
            T_gl_to_cv = utils.create_4x4_trans_mat_from_R_t(R_gl_to_cv)
            mv = torch.from_numpy(T_gl_to_cv).float().to(self.cfg.DEVICE)
            mvp = proj_mat @ mv
    ...
    def initialize_global_pose(self, data):        
        ...
        lmk2d_data = data["lmk2d"].cpu().numpy()    # (21, 2)
        ...
        lmk_x_data_palm = lmk_z_data_palm * (lmk2d_data_palm[:, 0] - cx) / fx # (n_palm_lmk,)
        lmk_y_data_palm = lmk_z_data_palm * (lmk2d_data_palm[:, 1] - cy) / fy # (n_palm_lmk,)
        ...
        if DEBUG_LOCAL:
            ...
            fx, fy, cx, cy = K[0, 0], K[1, 1], K[0, 2], K[1, 2]
            xyz_data = utils.depth_to_xyz_np(data["depth"].cpu().numpy(), data["depth"].cpu().numpy()>0, fx, fy, cx, cy)
        ...
        depth_ren, mask_ren = self.render_depth(vert, self.list_mvp[data["id_frame"]])    # (H, W)
        DEBUG_LOCAL = False
        if DEBUG_LOCAL:
        ...
        xyz_data = utils.depth_to_xyz(data["depth"], mask_depth_common, fx, fy, cx, cy)
    ...
    def compute_chamfer_loss(self, data, vert):
        ...
        # xyz_ren = utils.depth_to_xyz(depth_ren, mask_depth_common, fx, fy, cx, cy)
        # xyz_data = utils.depth_to_xyz(data["depth"], mask_depth_common, fx, fy, cx, cy)
        xyz_ren = utils.depth_to_xyz(depth_ren, mask_ren, fx, fy, cx, cy)
        xyz_data = utils.depth_to_xyz(data["depth"], mask_depth_data, fx, fy, cx, cy)
        xyz_data = xyz_data / 1000
            ...
            for data in self.dataset:
                ...
                if DEBUG:
                    with torch.no_grad():
                        ...
                        fx, fy, cx, cy = K[0, 0], K[1, 1], K[0, 2], K[1, 2]
                        mask_depth_data = data["rgba"][:, :, 3].bool()
                        xyz_data = utils.depth_to_xyz(data["depth"], mask_depth_data, fx, fy, cx, cy) / 1000
